...
(function(){
	...
	myapp.factory('authFactory', function($state, $cookies, $cookieStore, userFactory, loginService, $rootScope, $q, $injector, cacheFactory){
		var factory = {};
		...
		factory.login = function(user, pass){
			...
			userFactory.login(user, pass, function(data, error){
				if (!error && typeof data.data.token === 'object') {
					...
					// Carga la cache con los datos a utilizar en la app
