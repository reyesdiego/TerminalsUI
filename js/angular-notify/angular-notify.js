angular.module('cgNotify', []).factory('notify', ['$timeout', '$http', '$compile', '$templateCache', '$rootScope',
	function ($timeout, $http, $compile, $templateCache, $rootScope) {
		...
		var defaultTemplateUrl = 'view/angular-notify.html';
		var position = 'center';
		...
		var notify = function (args) {
			...
			$http.get(args.templateUrl, {cache: $templateCache}).then(function (response) {
				...
				var layoutMessages = function () {
					var j = 0;
					var currentY = startBottom;
					for (var i = messageElements.length - 1; i >= 0; i--) {
						var shadowHeight = 10;
						var element = messageElements[i];
						var height = element[0].offsetHeight;
						var bottom = currentY + height + shadowHeight;
						if (element.attr('data-closing')) {
							bottom -= 20;
						} else {
							currentY += height + verticalSpacing;
						}
						element.css('bottom', bottom + 'px').css('margin-bottom', '-' + (height + shadowHeight) + 'px').css('visibility', 'visible');
						j++;
					}
				};
				...
				throw new Error('Template specified for cgNotify (' + args.templateUrl + ') could not be loaded. ' + error.data);
			});
		...
		notify.config = function (args) {
			startBottom = !angular.isUndefined(args.startBottom) ? args.startBottom : startBottom;
